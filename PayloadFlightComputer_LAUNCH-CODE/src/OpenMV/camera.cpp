...
bool OpenMV::readData(CameraData* data)
{
    ...
    if (Wire1.requestFrom(CAMERA_I2C_ADDRESS, (int)sizeof(CameraData), true) == sizeof(CameraData))
    {
        for(uint8_t i = 0; i < sizeof(CameraData); i++) buffer[i] = Wire1.read();
...
GPSPoint OpenMV::getTargetPoint(CameraData const * data, float currLat, float currLong, float alt, float anglePitch, float heading){ //add in quaternion stuff here 
